...
`docs-complexity`, `docs-complexity-check`, `docs-examples`, `site`, `site-serve`, `bench`, `coverage`, `coverage-summary`, `coverage-check`, `javadoc`, `deps-updates`). `make verify` is what CI runs and must be green before a PR is opened. `make coverage` also
...
- The website (every page under `docs/` except `docs/design.md`, which is the decision log for contributors and is
  ...
  The concept is pattern matching; `switch` names only the Java syntax.
