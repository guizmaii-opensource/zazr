...
- Every subagent PR is reviewed by the coordinator: read the diff, verify the claims by running the
  build, tests and a probe or benchmark where a claim is measurable, and post the findings as a review
  with inline comments on the PR (GitHub refuses the request-changes state on a same-account PR: post a
