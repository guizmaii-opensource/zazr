...
- Internal types live in `.internal` packages (decided 2026-09-25, `docs/design.md` 3.1): collection internals in
  `dev.zazr.collection.internal`, the rest in `dev.zazr.internal`. They are `public` in name only
...
`docs-complexity`, `docs-complexity-check`, `docs-examples`, `docs-align`, `docs-align-check`, `site`, `site-serve`, `bench`, `coverage`, `coverage-summary`, `coverage-check`, `javadoc`, `deps-updates`). `make verify` is what CI runs and must be green before a PR is opened. `make coverage` also
fails when `zazr-core` drops below 95 % of lines or 95 % of branches (bundle level, `zazr-core` and `zazr-test`
tests together); new code comes with the tests that keep it there. Build and check scripts
...
- The website (every page under `docs/` except `docs/design.md`, which is the decision log for contributors and is
  not published) is written for a Java developer deciding whether and how to use Zazr (decided 2026-09-25, after a
  published paragraph of the log was unreadable). Short paragraphs, one idea each; a list when the content is a
  list; a heading per idea; a small example where it helps. No ticket numbers, no internal types or packages, no
  build flags, no history of how a decision was reached: that belongs in the decision log, which pages link to on
  GitHub. If a paragraph needs more than four or five lines to say one thing, split it or cut it. In the Java
  examples (site and README), local variables are declared with `var`, and where the type is what the example
  teaches it goes in an end-of-line comment (`var customer = customers.get("c-2"); // Option<Customer>`).
  Every link to another website opens in a new tab (the site does it for all external links, `docs/assets/external-links.js`).
  The concept is pattern matching; `switch` names only the Java syntax. Consecutive one-line statements of a block
  (no blank line, comment line or multi-line statement between them) read as columns: their `=` signs are aligned,
  and their trailing `//` comments start at one column, two spaces after the longest commented statement; a lone
  line is not padded. A code block shows 110 characters: when aligning would push a line past that, the group's type
  comments go on their own line above the declaration, or the redundant ones are dropped. `make docs-align` applies
  the rule to `docs/`, `README.md` and `skills/`, and `make docs-align-check` (run by `make verify` and CI) fails on
  a block that does not follow it. The copies of the snippets in the docs example tests are left to the Java
  formatter: `make docs-examples` compares them with the pages ignoring runs of spaces outside the string, char and
  text block literals.
- The Agent Skill in `skills/zazr/` (`SKILL.md` and `references/`) teaches coding assistants to write Zazr code. An API
  change updates it in the same PR, as for the website. Its java blocks are compiled and run by `SkillExamplesTest` and
  `SkillFunctionalJavaExamplesTest` (`make docs-examples` checks them), and `make vocabulary` covers it. It describes
