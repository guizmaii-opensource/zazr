...
- Once the coordinator is satisfied with a PR, an **independent review** is always launched: a fresh
  agent with no context from the session, briefed only with the repository, the PR and the review rules
  above, working in its own worktree, posting one review on GitHub with the verdict first. Its findings
  are triaged by the coordinator, the accepted ones go to the implementer, and the cycle (fix, reply
  in-thread, re-review) repeats. On PR #4 this second pair of eyes found three measured regressions the
  first review had accepted.
