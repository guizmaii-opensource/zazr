...
- `LinkedHashMap.Builder` is not the table's `HashMap.Builder` + `Vector.Builder` composite. A repeated key keeps the
  ...
  node types, which the CHAMP port (#117) replaces. The builder is therefore `LinkedHashMap.Puts`, the helper of the
  repeated-key decision, made public: an `ArrayList` of the keys in insertion order and a persistent `HashMap` from
  each key to its entry and position, made into the order `Vector` once, in `result()`. The fixed-arity `of`,
  ...
  each step; a builder-side lookup on the new trie is a follow-up once CHAMP is merged.
