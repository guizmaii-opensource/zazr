...
- **`check` fails the test; `evaluate` returns the result** (decided by the maintainer 2026-09-26, #179): writing
  `.assertIsSatisfied()` after every check was noise, so the common case is the default, as in zio-test where `check`
  inside a test is the assertion. `Check.check`/`checkN`/`checkAll` return nothing and throw the `AssertionError` of
