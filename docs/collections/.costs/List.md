...
| `appendAll` | <abbr class="cx cx-linear" title="O(n + m) for m appended elements: this List is copied. A List argument is shared, not copied; any other argument is copied into a new List first.">O(n + m)</abbr> | O(n + m) for m appended elements: this List is copied. A List argument is shared, not copied; any other argument is copied into a new List first. |
