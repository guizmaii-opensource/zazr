...
class TreeSetLawsTest extends SetLawsSuite<TreeSet<?>, TreeSet<Integer>> {
    ...
    CollectionSubject<Integer, TreeSet<Integer>> collection() {
        return new CollectionSubject<>(Arbitrary.treeSet(Arbitrary.integer()), TreeSet::ofAll, TreeSet::size, TreeSet::toList, false);
    }

    @Override
    BuilderLaws.CollectorSubject<Integer, TreeSet<Integer>> collector() {
        return new BuilderLaws.CollectorSubject<>(Arbitrary.list(Arbitrary.integer()), TreeSet.collector(), TreeSet::ofAll);
    }
}
